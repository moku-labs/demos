/**
 * @file board island — how the USER drives the board: the delegated interaction handler bodies
 * wired to selectors by events.ts. Grouped by feature — mutations (delete/edit/add card + column),
 * attachments (click-to-preview, upload), and drag-to-reorder. Mutations round-trip through `lib/api`
 * and the returning realtime patch updates state (see reconcile.ts).
 */
...
import { dropIndexInColumn, findAttachment, placeCardInColumn } from "../../lib/board-snapshot";
import { openPreview } from "./preview";
import { type BoardContext, DRAG_KEY, FALLBACK_TYPE } from "./types";

// ─── Mutations: cards & columns ──────────────────────────────────────────────
 ...
 * ```
 */
export function onCardAction(ctx: BoardContext, _event: Event, button: Element): void {
 ...
 * ```
 */
export async function onAddCard(ctx: BoardContext, event: Event, form: Element): Promise<void> {
 ...
 * ```
 */
export async function onAddColumn(ctx: BoardContext, event: Event, form: Element): Promise<void> {
  event.preventDefault();
 ...
 * ```
 */
export function onAttachmentClick(ctx: BoardContext, event: Event, link: Element): void {
  event.preventDefault();
 ...
 * ```
 */
export async function onAttach(ctx: BoardContext, _event: Event, input: Element): Promise<void> {
 ...
 * ```
 */
export function onDragStart(_ctx: BoardContext, event: Event, card: Element): void {
 ...
 * ```
 */
export function onDragOver(_ctx: BoardContext, event: Event): void {
  event.preventDefault();
}
 ...
 * ```
 */
export async function onDrop(ctx: BoardContext, event: Event, zone: Element): Promise<void> {
