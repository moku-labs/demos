-- 0001_init — initial Atlas schema (mirrors db/schema.sql; applied by `wrangler d1 migrations apply`).
-- Each plugin owns its tables (spec/15 §2). ON DELETE CASCADE chains the hierarchy; denormalized
-- scope columns (board_id, department_id, issue_id) keep customize/attachments purges and
-- board-snapshot loads to single indexed queries (no N+1).
