...
`api.ts` composes `createIssueCrud` + `createSubIssueApi` + `createPropertyApi` + `createMilestoneApi`
into the flat `Api` (the reason this is Complex, not Standard).
