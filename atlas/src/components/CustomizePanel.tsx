 ...
 * The element updates live. A popover on desktop, a bottom sheet on phones. Pure + SSR shared markup:
 * the Phase-C customize island re-renders it with the element's current colour/icon via
 * `h(CustomizePanel, props)` and wires the picks off the `data-action`/`data-value` hooks.
 */
...
export function CustomizePanel({ elementLabel, color = null, icon = null }: CustomizePanelProps) {
  return (
    <div data-customize-panel role="dialog" aria-label={`Customize ${elementLabel}`}>
