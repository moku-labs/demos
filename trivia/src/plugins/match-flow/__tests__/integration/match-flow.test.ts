...
describe("match-flow plugin integration", () => {
  ...
  it("re-baselines the armed steal while the SOLE stealer is unanswered, then stops (delivery heal — the stuck-grid regression)", {
    timeout: 25_000
  }, async () => {
    restoreFetch = mockFetch();
    // Two players → after the active answerer misses, exactly ONE stealer is eligible. This is the case
    // with NO built-in gap-heal: a sole stealer generates no later steal delta, so a replica that dropped
    // the lone `armed:true` frame never sees a sequence gap, never auto-resyncs, and (the phone has no
    // manual resync API) sits stranded on the disabled "get ready" grid until a manual reload — the
    // reported "steal locks before I choose, reload fixes it" bug. The host must re-deliver the armed
    // baseline every tick while that stealer is unanswered.
    const { host, controllers, qid } = await driveToQuestion(2, STABLE_QUESTION_TIMERS);
    const lead = controllers[0];
    ...
    // `armStealIfDue` is the ONLY caller of `stage.broadcast()` (the clock holds this exact reference),
    // so every call the spy records is a steal-window re-delivery — nothing else re-baselines.
    const rebaseline = vi.spyOn(host.stage, "broadcast");

    // Active locks WRONG (fixture `sha:0` → slot 1 correct) → the steal opens for the sole stealer.
    active?.controller.intent("answer-lock", { slot: 0, qid });
    await waitAdvancing(
      () => {
        expect(host.sync.read("steal")?.active).toBe(true);
        // Lead-in elapsed → the host armed the grid (the phone can now tap).
        expect(host.sync.read("steal")?.armed).toBe(true);
      },
      { timeout: 9000 }
    );

    // While the sole stealer is still unanswered, the host is re-delivering the armed baseline — the exact
    // heal a phone that missed the lone `armed:true` delta relies on (instead of a manual reload).
    expect(rebaseline).toHaveBeenCalled();

    // The stealer answers → the steal resolves to the reveal (everyone has tried).
    stealer?.controller.intent("answer-lock", { slot: 0, qid });
    await waitAdvancing(
      () => {
        expect(lead?.controller.read("match")?.phase).toBe("reveal");
    ...
    // Self-terminating: with the steal closed, nobody can be stuck arming, so the re-baseline stops (no
    // wasteful per-tick broadcast for the rest of the match). The count must be frozen across more ticks.
    const afterClose = rebaseline.mock.calls.length;
    await vi.advanceTimersByTimeAsync(300);
    expect(rebaseline.mock.calls.length).toBe(afterClose);

    rebaseline.mockRestore();
    await host.stop();
    await Promise.all(controllers.map(c => c.stop()));
  });
});
