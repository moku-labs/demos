...
3. **Server — ONE `@moku-labs/worker` app composing room's `hubPlugin` (atlas-style; full app control).**
   ...
   from `@moku-labs/worker`, where **`hubPlugin`** is imported from `@moku-labs/room/server` (room 0.3+'s
...
non-live question) so late retransmits are structurally inert. room 0.3.2 = sync-wire gap heal: a
unicast `sync.broadcast(peerId)` re-baselines at the CURRENT seq, and a replica that detects a delta
